...
public class GetMaxBatteryChargeCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher){
        dispatcher.register(Commands.literal("getmaxbattery").executes((context)-> {
            context.getSource().getPlayer().getCapability(BatteryCapabilityProvider.PLAYER_CHARGE).ifPresent((charge)-> {
                context.getSource().sendSuccess(()-> Component.literal("Max Charge: " + charge.getMaxCharge()), false);
            });
            return 0;
        }));
    }
