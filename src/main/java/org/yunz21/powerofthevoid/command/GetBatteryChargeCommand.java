...
public class GetBatteryChargeCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher){
        dispatcher.register(Commands.literal("getbattery").executes((context)-> {
            context.getSource().getPlayer().getCapability(BatteryCapabilityProvider.PLAYER_CHARGE).ifPresent((charge)-> {
                context.getSource().sendSuccess(()-> Component.literal("Charge: " + charge.getCharge()), false);
            });
            return 0;
        }));
    }
